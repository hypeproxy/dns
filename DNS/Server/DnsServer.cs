...
namespace DNS.Server
{
	public class DnsServer : IDisposable
	{
		...
		public DnsServer(IRequestResolver resolver, IPEndPoint endServer) : this(new FallbackRequestResolver(resolver, new UdpRequestResolver(endServer)))
		{
		}

		public DnsServer(IRequestResolver resolver, IPAddress endServer, int port = DEFAULT_PORT) : this(resolver, new IPEndPoint(endServer, port))
		{
		}
		...
		public DnsServer(IPEndPoint endServer) : this(new UdpRequestResolver(endServer))
		{
		}
		...
		public DnsServer(string endServer, int port = DEFAULT_PORT) : this(IPAddress.Parse(endServer), port)
		{
		}
		...
		private async void HandleRequest(byte[] data, IPEndPoint remote)
		{
			...
			try
			{
				...
				await udp.SendAsync(response.ToArray(), response.Size, remote).WithCancellationTimeout(TimeSpan.FromMilliseconds(UDP_TIMEOUT)).ConfigureAwait(false);
			}
			catch (SocketException e)
			{
				OnError(e);
